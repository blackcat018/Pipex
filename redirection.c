...
/*   Updated: 2025/02/24 02:00:46 by moel-idr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int open_fds(t_vabs *pipex)
{
    ...
    pipex->outfile = open(pipex->av[pipex->ac - 1], O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (pipex->outfile == -1)
    {
        perror("open outfile");
        close(pipex->infile);
...
void handle_child(t_vabs *pipex, int index, int prev_pipe_fd, int next_pipe_fd)
{
    char **cmd;
